...
namespace WebApp.Models.EntityManager
{
    public class UserManager
    {
        public object AuthorizationAttribute { get; private set; }
        ...
        public void AddUserAccount(UserSignUpView user)
            {
                using (DemoDBEntities1 db = new DemoDBEntities1())
                {
                    ...
                    SU.PasswordEncryptedText = Crypto.HashPassword(user.Password);
                    ...
                    //{
                        ...
                        SUR.IsActive = true;
                        SUR.RowCreatedSYSUserID = GetUserID(user.LoginName) > 0 ? GetUserID(user.LoginName) : 1;
                        SUR.RowModifiedSYSUserID = GetUserID(user.LoginName) > 0 ? GetUserID(user.LoginName) : 1;
        ...
        public string[] GetAllRole()
        {
            ...
            using (DemoDBEntities1 db = new DemoDBEntities1())
            {
                ...
                int i = roles.Count();
                int j = 0;
                foreach (LOOKUPAvailableRole role in roles)
                {
                    roless[j] = role.RoleName;
                    j++;
                    if (j > roles.Count - 1)
                    {
                        return roless;
